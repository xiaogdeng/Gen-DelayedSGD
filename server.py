...
from models import resnet18, MNISTNet, init_params, \
    LinearNet_CovType, LinearNet_ijcnn1, LinearNet_rcv1, LinearNet_rcv1_multi, LinearNet_gisette
from libsvm_data import IJCNN1, CovTypeBinary, RCV1Binary, RCV1MultiClass, GISETTE
...
def load_dataset(name, location, train=True):
    ...
    if name == 'mnist':
        ...
        # of features: 22
        train_dataset = IJCNN1("train", download=True, data_root=location)
        test_dataset = IJCNN1("test", download=True, data_root=location)
    ...
    elif name == 'rcv1_multi':
        # of classes: 53
        # of data: 15,564 / 518,571 (testing)
        # of features: 47,236
        train_dataset = RCV1MultiClass("train", download=True, data_root=location)
        test_dataset = RCV1MultiClass("test", download=True, data_root=location)

    elif name == 'gisette':
        # of classes: 2
        # of data: 6,000 / 1,000 (testing)
        # of features: 5,000
        train_dataset = GISETTE("train", download=True, data_root=location)
        test_dataset = GISETTE("test", download=True, data_root=location)

    elif name == 'covtype':
        # of classes: 2
        # of data: 581,012
        # of features: 54
        CovType_data = CovTypeBinary(download=True, data_root=location)
        generator = torch.Generator()
        generator.manual_seed(0)
        num_train = int(len(CovType_data) * 3 / 5)
        num_test = len(CovType_data) - num_train
        train_dataset, test_dataset = torch.utils.data.random_split(CovType_data, [num_train, num_test], generator)
...
def load_model(name=None, dataset=None):
    ...
    if name == 'resnet' and dataset == 'cifar10':
        ...
        net = MNISTNet()
    elif name == 'linearnet_ijcnn1':
        net = LinearNet_ijcnn1()
    elif name == 'linearnet_covtype':
        net = LinearNet_CovType()
    elif name == 'linearnet_rcv1':
        net = LinearNet_rcv1()
    elif name == 'linearnet_rcv1_multi':
        net = LinearNet_rcv1_multi()
    elif name == 'linearnet_gisette':
        net = LinearNet_gisette()
    else:
        raise ValueError(name + ' is not known.')
    init_params(net)
...
class ParameterServer:
    ...
    def evaluation(self, epoch):
        ...
        print("eval gen error", test_loss - train_loss)
        print("eval gen error-abs", np.abs(test_loss - train_loss))
    ...
    def step(self):
        ...
        for worker_ in self.workers:
            start_time = time.time()
            worker_loss, batch_size_ = worker_.compute_gradients(self.running_itr)
            ...
            self.tb_writer.add_scalar(f'TrainLoss/worker_{[worker_.id]}', worker_loss, self.running_itr)
            self.tb_writer.add_scalar(f'Delay/worker_{[worker_.id]}', worker_.delay, self.running_itr)
...
if __name__ == '__main__':
    ...
    if args.delay_type == 'random':
        for id_ in range(args.num_workers):
            if id_ == 0:
                worker_delays.append(max_delay)
            elif id_ == 1:
                worker_delays.append(1)
            else:
                worker_delays.append(np.random.randint(1, 1 + max_delay))
    elif args.delay_type == 'fixed':
        worker_delays = [max_delay for _ in range(args.num_workers)]
    else:
        assert False, 'not implemented delay type'

    task_name = f'{args.model}_{args.dataset}/runs_worker-{args.num_workers}_{args.delay_type}-{args.delay}delay_bs-' \
